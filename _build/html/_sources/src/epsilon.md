...
Abramowitz and Stegun use the forms $Z(\phi\backslash\alpha)$ and $Z(u|m)$ to distinguish the two cases. Here $\sin^2\alpha=m$. They seem to use the same convention, but not mentioning "Jacobi's epsilon" in the form $\mathscr E(u,m)=E(u)$, dropping the reference to m.
